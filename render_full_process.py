...
def render_set_fixcam(model_path, name, iteration, views, gaussians, transitions, trajs, pipeline, background,multiview_video, fname='video_rgb.mp4', funcs=None, scales=None, appears=[], pre_scale=False, cam_idx=25):
    ...
    for idx in tqdm(range (fnum)):
        ...
        # idx = int(idx*(compose_end - compose_start) + compose_start*fnum)  # project to the specific interval
        ...
        for transition_idx, _ in enumerate(trajs.trans_list):
            transition_start, transition_end = trajs.trans_period[transition_idx][0], trajs.trans_period[transition_idx][1]
            if transition_start <= (idx / fnum) and transition_end >= (idx / fnum):
                for t_idx in trajs.trans_list[transition_idx]: trans_list[t_idx] = 1
                # Optional: set transition iter rates will make the results better
                iter_rate = ((idx / fnum)-transition_start)/(transition_end-transition_start)
...
def render_sets(dataset : ModelParams, hyperparam, opt, trajs, iteration : int, pipeline : PipelineParams, output_path : str, skip_train : bool, skip_test : bool, skip_video: bool,multiview_video: bool):
    ...
    with torch.no_grad():
        ...
        transitions = [Transition(training=False).to("cuda") for _ in dataset.cloud_path]
